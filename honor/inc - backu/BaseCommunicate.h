...
// 功能: 发送一条缓冲区数据。与 RequestBuffer 接口差异为，本接口内部针对X芯片平台特有的延迟响应包做特殊处理，即第一条响应包并不立即返回，捕获第二条响应包后汇总一并返回
// 参数: const char* addr 的定义：网口时，ip:port[:protocol]，举例：127.0.0.1:8080
//       Win  平台下串口const char* addr 定义: port number[:baud rate][:protocol], 可选支持波特率设置，默认 57600，举例1：24，举例2：23:9600
//       Linux平台下串口const char* addr 定义：device path，举例：/dev/ttyS0
//       可选[:protocol]，用于指定预定义的特殊的组包方式(目前支持 hdlc、usbbox、hdlc_usbbox 协议)。比如192.168.1.105:1000:usbbox，或者36:9600:usbbox
...
// 参数: void* callback LOG打印的回调函数集对象的指针，接口内部不用关心数据结构
// 返回: 失败返回-1，成功返回0
BASECOMMUNICATE_API int   RequestBufferSimlock(const char* addr, const char* req, int reql, const char* expect, int expl, char* rsp, int size, int& rspl, void* callback);

// 功能: 只有定制工位的谷歌证书和PKI证书两个测试项允许使用，其它工位的装备和测试项禁止使用此接口！！！
// 返回: 失败返回-1，成功返回0
BASECOMMUNICATE_API int   RequestForbidden(const char* addr, const char* req, const char* expect, char* rsp, int size, void* callback);
